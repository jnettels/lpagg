...
### --- Script options ---------------------------------------------------------
#base_folder = u'W:\\02_Projekte\\6xx\\695 FutureSuN_BMWi\\03_Bearbeitung\\AP4_Transformation\\AP404_Konzepte für zukünftige Systemlösungen\\Lastprofile\\VDI 4655\\Berechnung'
#base_folder = 'V:\MA\2_Projekte\SIZ10015_futureSuN\4_Bearbeitung\AP4_Transformation\AP404_Konzepte für zukünftige Systemlösungen\Lastprofile\VDI 4655\Berechnung'
base_folder = 'V:\\MA\\2_Projekte\\SIZ10015_futureSuN\\4_Bearbeitung\\AP4_Transformation\\AP404_Konzepte für zukünftige Systemlösungen\\Lastprofile\\VDI 4655\\Berechnung'

holiday_file        = os.path.join(base_folder,'Typtage','Feiertage.xlsx')
energy_factors_file = os.path.join(base_folder,'Typtage','Faktoren für Energiebedarf je Typtag.xlsx')
...
datetime_start = pd.datetime(*settings['start']) # '*' reads the list as args
...
if interpolation_freq < pd.Timedelta('15 minutes'):
    print('Warning! Chosen interpolation intervall '+str(interpolation_freq)+\
          ' changed to 15 min. MFH load profiles are available in 15 min '+\
          'steps and interpolation to smaller time intervals is not (yet) '+\
          'implemented.')
    interpolation_freq = pd.Timedelta('15 minutes')
...
### For summer days, no distinction has been made in terms of cloud amount.
### So we need to replase 'heiter' and 'bewölkt' with 'X'
...
### ----------------------------------------------------------------------------
print('Read in reference load profile factors and match them to "typtag" keys for each timestep')

### Create list of all 'typtag' load-curve files:
typtage_combinations = ['SSX','SWX','USB','USH','UWB',
                        'UWH','WSB','WSH','WWB','WWH']
house_types          = ['EFH','MFH']
...
### I.e. they are labeled 'left', while the IGS-Weatherdata is labeled 'right'!
...
### In the next step, the values are summed up into the same time intervals as
### the weather data, and the label is moved to the 'right'
### (each time stamp now describes the data in the interval before)
typtage_level_values = typtage_df.index.get_level_values
typtage_df = typtage_df.groupby(
    [typtage_level_values(i) for i in [0,1]] +
    [pd.Grouper(freq=interpolation_freq, level=-1, label='right', closed='left')]
    ).sum()
...
multiindex     = pd.MultiIndex.from_product(iterables, names=['energy', 'house'])
load_profile_df = pd.DataFrame(index = weather_data.index, columns = multiindex)
...
for j, date_obj in enumerate(weather_data.index):
    ...
    # Compare time stamps in the typtage_df of the matching house and typtag
    ...
    print('{:5.1f}% done'.format(j/total*100), end='\r') # print progress info


### Debugging: The daily sum of each energy factor type must be '1':
# print load_profile_df.resample('D', label = 'left', closed='right').sum()


### ----------------------------------------------------------------------------
### VDI 4655 - Step 3: Load houses and generate their load profiles
###
### In the end, we will multiply the energy factors contained in load_profile_df
### DataFrame for each time step of the weather_data with the corresponding
### daily energy demand of each house we want to simulate. This will yield
### the DataFrame load_curve_houses, which contains the actual energy demand
### of each house in each time step.
### ----------------------------------------------------------------------------

### (6) Application of guideline:
### ----------------------------------------------------------------------------
### The following numbered sections are the implementations of the corresponding
### sections in the VDI 4655.
...
for house_name in houses_list:
    ...
    if house_type == 'EFH':
        ...
        if      N_Pers <  3:    W_a    = N_Pers * 2000 # kWh
        elif N_Pers <= 6:    W_a    = N_Pers * 1750 # kWh
        else:                W_a    = N_Pers * 1500 # kWh
    ...
    ### If W_a and/or Q_TWW_a were already defined by the user in the yaml file,
    ### we use those values instead of the calculated ones:
...
### radiation year (TRY) regions, house types and 'typtage'. In VDI 4655, these
### are the tables 10 to 24.
...
multiindex = pd.MultiIndex.from_product(iterables, names=['house', 'energy'])
...
for house_name in houses_list:
    ...
    # Savety check:
    if not TRY in energy_factors_df.index.get_level_values(0):
        print('Error! TRY '+str(TRY)+' not contained in file '+energy_factors_file)
    ...
    for typtag in typtage_combinations:
        F_Heiz_TT     = energy_factors_df.loc[TRY,house_type,'F_Heiz_TT'][typtag]
        Q_Heiz_a     = houses_dict[house_name]['Q_Heiz_a']
        Q_Heiz_a     = Q_Heiz_a * config_dict.get('adjustment_factors',dict()).get('f_Q_Heiz',1)

        F_el_TT        = energy_factors_df.loc[TRY,house_type,'F_el_TT'][typtag]
        W_a            = houses_dict[house_name]['W_a']
        W_a         = W_a * config_dict.get('adjustment_factors',dict()).get('f_W',1)

        F_TWW_TT    = energy_factors_df.loc[TRY,house_type,'F_TWW_TT'][typtag]
        Q_TWW_a        = houses_dict[house_name]['Q_TWW_a']
        Q_TWW_a     = Q_TWW_a * config_dict.get('adjustment_factors',dict()).get('f_Q_TWW',1)
        ...
        daily_energy_demand_houses.loc[house_name,'Q_Heiz_TT'][typtag] = Q_Heiz_TT
        daily_energy_demand_houses.loc[house_name,'W_TT'][typtag]        = W_TT
        daily_energy_demand_houses.loc[house_name,'Q_TWW_TT'][typtag]  = Q_TWW_TT
...
multiindex = pd.MultiIndex.from_product(iterables, names=['house', 'energy'])
load_curve_houses = pd.DataFrame(index=weather_data.index, columns=multiindex)
    ...
    for house_name in houses_list:
        ...
        for i, energy_factor_type in enumerate(energy_factor_types):
            ...
            load_curve_houses.loc[date_obj][house_name,energy_demand_type] = \
                load_profile_df.loc[date_obj][energy_factor_type,house_type] * \
                daily_energy_demand_houses.loc[house_name,energy_demand_type][typtag]
...
### Sum up the energy demands of all houses and store that in the weather file
print('Sum up the energy demands of all houses')

### By grouping the 'energy' level, we can take the sum of all houses
load_curve_houses_sum = load_curve_houses.groupby(level='energy', axis=1).sum()
...
total = float(len(weather_data.index))
for j, date_obj in enumerate(weather_data.index):
    T_a     = weather_data.loc[date_obj]['TAMB']
    ...
    if (T_a < T_i): # only calculate if heating is necessary
        ...
        M_dot_Heiz    = Q_dot_Heiz/(c_p*(T_VL_Heiz - T_RL_Heiz))*3600.0    # kg/h
    ...
    M_dot_TWW    = Q_dot_TWW/(c_p*(T_VL_TWW - T_RL_TWW))*3600.0    # kg/h
    ...
    Q_loss = 0
    if (M_dot > 0):
        d_i     = config_dict['Verteilnetz']['diameter_inner']        # m
        ...
        # print Q_dot_Heiz, Q_dot_TWW, Q_dot_loss
...
sum_list = ['Q_Heiz_TT', 'Q_TWW_TT', 'W_TT', 'Q_loss']
weather_daily_sum = weather_data[sum_list].resample('D',
                        label='left', closed='right').sum()

print('Calculations completed')
print('Monthly energy sums in kWh:')
weather_montly_sum = weather_daily_sum.resample('M',
                        label='right', closed='right').sum()
print(weather_montly_sum)
print()
print('Annual energy sums in kWh:')
print(weather_montly_sum.resample('A', label='right', closed='right').sum())
print()

### Display a plot on screen for the user
if bool_show_plot == True:
    ...
    plt.legend()
    plt.show(block=False) # Starts showing the plot, without blocking the script

### Print the results file
if settings.get('print_columns',None) != None:
...
plt.show() ### The script will be blocked until the user closes the plot window